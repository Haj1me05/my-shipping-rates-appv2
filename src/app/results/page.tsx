...
import type { RateRequest, ServiceSpeed } from '@/types/domain';
...
export default function ResultsPage() {
  ...
  // Extract selected shipping speed from request to pre-filter results
  const selectedSpeed = useMemo(() => {
    try {
      const requestData = searchParams.get('request');
      if (!requestData) return undefined;
      const request: RateRequest = JSON.parse(decodeURIComponent(requestData));
      return request.shippingOptions?.speed;
    } catch {
      return undefined;
    }
  }, [searchParams]);
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <Suspense fallback={<ResultsSkeletonLoader />}>
          <RatesDisplay ratesPromise={ratesPromise} selectedSpeed={selectedSpeed} />
        </Suspense>
      </div>
    </div>
  );
